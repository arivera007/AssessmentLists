...
def all_odd(numbers):
    ...
    #return filter(lambda number: number % 2 != 0, numbers)          # Filtered out what has a remainder different from 0 when divided by 2.
    return [number for number in numbers if number % 2 != 0]         # or generating a comprehention list 
...
def print_indices(items):
    ...
    for i in range(len(items)):                 # Traverse the entire size of the list by index
        print str(i) + ' ' + str(items[i])      # Print index and the item.
...
def every_other_item(items):
    ...
    return items[0::2]          # Slicing list starting position 0 and stepping by 2 instead of default 1.
...
def largest_n_items(items, n):
    ...
    if n==0: return []                  # If numbers ask is 0, then return an empty list
    else:
        return sorted(items)[-n:]       # if not, sort list first and then slice the last "n" elements asked. Default sort is ascending.
